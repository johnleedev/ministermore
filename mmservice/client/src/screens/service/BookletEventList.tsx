...
import type { EventBookletItem } from './serviceManageTypes';
import './ServiceList.scss';

export default function BookletEventList() {
  ...
  const createUrl = `${MainSiteURL.replace(/\/$/, '')}/service/bookleteventpay`;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`${ServiceAPIURL}/bookleteventcreate/getUserBooklets/${account}`)
      .then((res) => {
        if (cancelled) return;
        if (res.data?.success && Array.isArray(res.data.data)) {
          setEventBookletList(res.data.data);
        } else {
          setEventBookletList([]);
        }
      })
      .catch((error) => {
        console.error('모바일행사전단지 목록 가져오기 실패:', error);
        if (!cancelled) setEventBookletList([]);
      })
      .finally(() => {
  ...
  return (
    <div className="service-admin__list-page">
      <div className="service-admin__list-header">
        <h1 className="service-admin__list-title">모바일행사전단지 관리</h1>
            ...
            <div key={item.id} className="service-admin__posting-item">
              <div className="service-admin__posting-header">
                <div className="service-admin__posting-title">
                  <span className="service-admin__category-tag">행사 전단지</span>
                  <h3 className="service-admin__posting-name">{item.eventName || '행사명 없음'}</h3>
              ...
              <div className="service-admin__posting-info">
                <div className="service-admin__info-row">
                  <span className="service-admin__info-label">행사명:</span>
                  <span className="service-admin__info-value">{item.eventName || '-'}</span>
                </div>
                <div className="service-admin__info-row">
                  <span className="service-admin__info-label">일시:</span>
                  <span className="service-admin__info-value">{item.date || '-'}</span>
                </div>
                <div className="service-admin__info-row">
                  <span className="service-admin__info-label">장소:</span>
                  <span className="service-admin__info-value">{item.place || '-'}</span>
                </div>
