...
import ServiceAPIURL from '../../../../../ministermore/client/src/ServiceAPIURL';
...
export default function HomeinappNotificationMain() {
  ...
  const fetchNotificationSummary = async (churchId: string) => {
    const res = await axios.get(`${ServiceAPIURL}/homeinappmain/notifications/summary/${encodeURIComponent(churchId)}`);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${ServiceAPIURL}/homeinappmain/getChurchForUser/${encodeURIComponent(userAccount)}/${encodeURIComponent(churchId)}`
      )
      .then((res) => {
        if (!mounted) return;
        const payload = res.data;
  ...
  useEffect(() => {
    ...
    let mounted = true;
    axios
      .get(`${ServiceAPIURL}/homeinappmain/users/${encodeURIComponent(churchId)}`)
      .then((res) => {
        if (!mounted) return;
        const payload = res.data;
        if (!payload?.success) return;
  ...
  const onSend = async () => {
    ...
    try {
      setIsSending(true);
      const response = await axios.post(`${ServiceAPIURL}/homeinappmain/sendPushByChurch`, {
        churchId,
  ...
  const onDeleteHistoryItem = async () => {
    ...
    try {
      await axios.delete(
        `${ServiceAPIURL}/homeinappmain/notifications/${encodeURIComponent(cid)}/${bodyModal.id}`,
      );
      setBodyModal(null);
