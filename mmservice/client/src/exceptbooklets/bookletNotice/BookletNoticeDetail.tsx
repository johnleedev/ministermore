...
export default function BookletNoticeDetailPage() {
  ...
  const fetchPosts = async () => {
    if (!ID) return;
    try {
      const resBooklet = await axios.post(`${ServiceAPIURL}/bookletnoticecreate/getdatabookletspart`, { id: ID });
      ...
      const resServers = await axios.post(`${ServiceAPIURL}/bookletnoticecreate/getdataserverspart`, { id: ID });
      setServersData(resServers.data || []);

      const resSermon = await axios.post(`${ServiceAPIURL}/bookletnoticecreate/getdatasermonpart`, { id: ID });
      setSermonVideos(resSermon.data || []);

      try {
        const resGallery = await axios.post(`${ServiceAPIURL}/bookletnoticecreate/getdatagallerypart`, { id: ID });
  ...
  const mainHeroImageSrcs = useMemo(() => {
    ...
    return parseMainImageNameFromDb(postData.imageMainName)
      .filter(Boolean)
      .map((name) => `${ServiceAPIURL}/images/bookletnotice/mainimages/${name}`);
