...
// 게시글 조회시, 조회수 증가시키기
router.post('/usedpostsviews', (req, res) => {
  const { postId } = req.body;
  boarddb.query(`
    UPDATE usedPosts SET views = views + 1 WHERE id = ${postId}
...
const storage = multer.diskStorage({
  destination(req, file, done) { 
    done(null, 'build/images/postimage/used');
  }, 
...
// 게시글 첨부사진 전체 삭제하기
router.post('/useddeleteimages', async (req, res) => {
  const { postId, userAccount, images } = req.body;
  try {
    // 각 이미지를 삭제하는 비동기 함수
    const deleteImage = (image) => {
      return new Promise((resolve, reject) => {
        const imagePath = `build/images/postimage/used/${image}`;
    ...
    boarddb.query(`
      UPDATE usedPosts SET images = NULL WHERE id = '${postId}' AND userAccount = '${userAccount}';
      `,function(error, result){
      if (error) {throw error}
      if (result.affectedRows > 0) {
        res.send(true);
...
// 게시글 삭제하기
router.post('/useddeletepost', async (req, res) => {
  
  const { postId, userAccount, images } = req.body;

  boarddb.query(`DELETE FROM usedComments WHERE post_id = '${postId}' AND userAccount = '${userAccount}';`);
  boarddb.query(`DELETE FROM usedIsliked WHERE post_id = '${postId}' AND userAccount = '${userAccount}';`);
  ...
  try {
    ...
    boarddb.query(`
      DELETE FROM usedPosts WHERE id = '${postId}' AND userAccount = '${userAccount}';
      `,function(error, result){
      if (error) {throw error}
      if (result.affectedRows > 0) {
        res.send(true);
...
// 좋아요 데이터 가져오기
router.get('/getusedisliked/:postID', (req, res) => {
  var postId = parseInt(req.params.postID);
  boarddb.query(`
    SELECT * FROM usedIsliked WHERE post_id = '${postId}';
  `, function(error, result) {
    if (error) throw error;
    if (result.length > 0) {
      res.send(result);
...
// 게시글 좋아요 버튼, 토글 관리
router.post('/usedislikedtoggle', (req, res) => {
  
  const { isLiked, postId, userAccount } = req.body;

  if (isLiked === false) {
    boarddb.query(`
    UPDATE usedPosts SET isLiked = isLiked + 1 WHERE id = ${postId};
    `,function(error, result){
    if (error) {throw error}
    if (result.affectedRows > 0) {
      boarddb.query(`
      INSERT IGNORE INTO usedIsliked (post_id, isliked, userAccount) VALUES 
    ...
    if (result.affectedRows > 0) {
      boarddb.query(`
      DELETE FROM usedIsliked WHERE post_id = ${postId} AND userAccount = '${userAccount}';
      `,function(error, result){
      if (error) {throw error}
      if (result.affectedRows > 0) {
        res.send(true); res.end();
...
// 특정 게시물의 댓글 목록 가져오기
router.get('/getallusedcomments/:postId', (req, res) => {
  var postId = parseInt(req.params.postId);
  boarddb.query(`
  SELECT * FROM usedComments WHERE post_id = '${postId}'
  `, function(error, result){
  if (error) {throw error}
  if (result.length > 0) {
    res.send(result);
...
// 댓글 입력하기
router.post('/usedcommentsinput', (req, res) => {
  const { postId, commentText, date, userAccount, userNickName} = req.body;
  
  boarddb.query(`
  INSERT IGNORE INTO usedComments (post_id, content, userAccount, userNickName, date) VALUES 
...
// 댓글 삭제하기
router.post('/usedcommentdelete', (req, res) => {
  const { commentId, postId, userAccount } = req.body;

  boarddb.query(`
  DELETE FROM usedComments WHERE id = '${commentId}' AND post_id = '${postId}' AND userAccount = '${userAccount}';
  `,function(error, result){
  if (error) {throw error}
  if (result.affectedRows > 0) {
    res.send(true);
...
// 특정 유저 댓글 작성 여부 확인하기
router.post('/checkusedisposting', (req, res) => {
...
router.post('/usedadminmovepost', handleAdminMovePost({ db: boarddb }));
