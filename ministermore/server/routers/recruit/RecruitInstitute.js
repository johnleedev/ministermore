...
router.get('/getrecruitdata/:page', async (req, res) => {
  ...
  const countQuery = `
    SELECT COUNT(*) AS totalCount FROM recruitInstitute;
  `;
...
router.post('/getrecruitdatapart', async (req, res) => {
  var { id } = req.body;
  const query = `
    SELECT * FROM recruitInstitute WHERE id = '${id}';
  `;
...
router.post('/recruitsearchlist', async (req, res) => {
  ...
  // 디버깅: 검색 요청 데이터 확인
  console.log('검색 요청 데이터:', { location });
  
  // 디버깅: 강원 관련 데이터 확인
  if (location.includes('강원')) {
    console.log('강원 검색 요청됨:', location);
    recruitdb.query('SELECT DISTINCT location FROM recruitInstitute WHERE location LIKE "%강원%"', (err, result) => {
      if (!err) {
  ...
  // 직무 관련 검색 제거됨 (클라이언트에서 더 이상 사용하지 않음)
  // if (sort.length > 0) {
  ...
  const query = `SELECT * FROM recruitInstitute ${whereSQL};`;
