...
export default function UsedDetail () {
  ...
  const fetchDatas = async () => {
    const resComment = await axios.get(`${MainURL}/usedboard/getallusedcomments/${propsData.id}`)
    if (resComment.data) {
      const copy = resComment.data;
      copy.reverse();
      setCommentsList(copy);
    }
    const resIsliked = await axios.get(`${MainURL}/usedboard/getusedisliked/${propsData.id}`)
    if (resIsliked.data) {
  ...
  const handleislikedtoggle = async () => {
    axios 
      .post(`${MainURL}/usedboard/usedislikedtoggle`, {
        postId : propsData.id,
  ...
  const registerComment = async () => {
    axios 
      .post(`${MainURL}/usedboard/usedcommentsinput`, {
        postId : propsData.id,
  ...
  const deleteComment = (item:any) => {
    axios
      .post(`${MainURL}/usedboard/usedcommentdelete`, {
        commentId : item.id,
        postId : item.post_id,
        userAccount : userData.userAccount
      })
  ...
  const deletePost = () => {
    axios
      .post(`${MainURL}/usedboard/useddeletepost`, {
        postId : propsData.id,
          ...
          <div className="subpage__main__title">
            ...
            <div style={{display:'flex'}}>
              ...
              }
              {
                ...
                <div className='postBtnbox'
                  onClick={()=>{
                    navigate('/community/usedpost', {state : {sort:propsSort, menuNum:propsMenuNum}});  
                  }}
                >
                  <p>글쓰기</p>
                </div>
            ...
            <div className="view_content">
              ...
              { images.length > 0 &&
                images.map((item:any, index:any)=>{
                  return (
                    <img src={`${MainURL}/images/postimage/used/${item}`} key={index}/>
                  )
                })
              }
              </div>
