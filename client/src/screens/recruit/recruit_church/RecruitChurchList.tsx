...
export default function RecruitChurchList (props:any) {
  ...
  const [activeTab, setActiveTab] = useState('직무'); // '직무' 또는 '지역'
  ...
  const filterByAllConditions = (data: any[]) => {
    return data.filter((item: any) => {
      // sort 조건 - title 필드에서 검색
      const sortOk = selectedSort.length === 0 || selectedSort.some(s => item.title && item.title.includes(s));
  ...
  const handleSearch = async () => {
    ...
    const res = await axios.post(`${MainURL}/recruitchurch/recruitsearchlist`, {
      sort: selectedSort,
      location: selectedLocation,
      religiousbody: [],
    })
  ...
  const handleUnifiedSearch = async () => {
    if (!searchWord && selectedSort.length === 0 && selectedLocation.length === 0) {
    ...
    const res = await axios.post(`${MainURL}/recruitchurch/recruitsearchunified`, {
  ...
  const performUnifiedSearch = async ({
    ...
    if (res.data.resultData) {
      ...
      const filtered = res.data.resultData.filter((item: any) => {
        const sortOk = ss.length === 0 || ss.some(s => item.title && item.title.includes(s));
  ...
  const handleItemClick = (itemId: string) => {
    ...
    navigate(`/recruit/recruitchurchdetail?id=${itemId}`, { state: { recruitState } });
    window.scrollTo(0, 0);
  };
        ...
        <div className="subpage__main">
          ...
          <div className="subpage__main__title">
            ...
            <div className='postBtn'
              onClick={()=>{
                if (isLogin) {
                  navigate('/recruit/recruitchurchpost');
                } else {
                  alert('로그인이 필요합니다.');
                }
              }}
          ...
          <div className='checkInputCover Recruit'>
            {(activeTab === '직무' ? churchJobSorts : cityNames).map((item:any, index:any) => (
              <div
          ...
          <div className="subpage__main__selectedwords">
            ...
            <div className="subpage__main__selectedwords__list">
              {[...selectedSort, ...selectedLocation].length === 0 ? (
                <span className="subpage__main__selectedwords__none">없음</span>
              ) : (
                [...selectedSort, ...selectedLocation].map((item, idx) => (
                  <span
                    key={idx}
            ...
            <div className="main__content">
                ...
                <div className="recruit__wrap--category">
                  ...
                  <div className="recruit__wrap--menu">
                    ...
                    <div className='recruit_menu_box'>
                      <p className='recruit_menu_text'>지역</p> 
                    </div>
                    <div className='recruit_menu_box'>
                          ...
                          <div className="recruit__content">
                            ...
                            <div className='recruit__sort_box'>
                              <div className='recruit__religiousbody_box'>
                                <img src={`${MainURL}/siteimages/religiousbody/${item.religiousbody}.jpg`} />
