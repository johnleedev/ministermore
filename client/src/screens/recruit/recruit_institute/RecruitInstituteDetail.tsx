...
export default function RecruitDetail() {
  ...
  const addressAPI = async (addressQuery: any) => {
    try {
      const response = await axios.post(`${MainURL}/recruitinstitute/geocode`, {
  ...
  const fetchPosts = async () => {
    const res = await axios.post(`${MainURL}/recruitinstitute/getrecruitdatapart`, {
      id : ID
    })
    if (res.data) {
          ...
          <section style={{marginBottom:'50px'}}>
            <div className="main_title_row" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
              ...
              <div style={{
                ...
                onClick={() => {
                  window.scrollTo(0, 0);
                  navigate('/recruit/recruitinstitutelist', { state: { recruitState } });
                }}
                onMouseEnter={(e) => {
