...
import { applydocSubSort, applyhowSubSort, careerSubSort, dawnPraySubMenu, hourSortOption, insuranceSubSort, minuteSortOption, 
        ...
        welfareSubSort, 
        workdaySubMenu, workDayWeekSortOption } from './RecruitMinisterPostData';
...
export default function RecruitPost (props:any) {
  ...
  const registerPost = async () => {
    ...
    try {
      const res = await axios.post(`${MainURL}/recruitminister//postsrecruit`, formData, {
        headers: {
                  ...
                  workday.map((item: any, index: number) => {
                    return (
                      <div key={index}>
                        ...
                        <div className='checkTextRow' style={{display:'flex'}}>
                        {
                          workdaySubMenu.map((subItem:any, subIndex:any)=>{
                            return (
                  ...
                  dawnPray.map((item: any, index: number) => {
                    return (
                      <div key={index}>
                        ...
                        <div className='checkTextRow' style={{display:'flex'}}>
                        {
                          dawnPraySubMenu.map((subItem:any, subIndex:any)=>{
                            return (
                              <div key={subIndex} className='checkTextBox'
                                onClick={() => {
                                  const copy = [...dawnPray];
                  ...
                  insurance.map((item: any, index: number) => {
                    return (
                      <div key={index}>
                        ...
                        <div className='subTextBox'>
                          {
                            insuranceSubSort.map((subItem: string, subIndex: number) => {
                              const isSelected = item.content.split(', ').includes(subItem);
                              return (
                                <p key={subIndex}
                                  className={`subText ${isSelected ? 'selected' : ''}`}
                  ...
                  severance.map((item: any, index: number) => {
                    return (
                      <div key={index}>
                        ...
                        <div className='subTextBox'>
                          {
                            severanceSubSort.map((subItem: string, subIndex: number) => {
                              const isSelected = item.content.split(', ').includes(subItem);
                              return (
                                <p key={subIndex}
                                  className={`subText ${isSelected ? 'selected' : ''}`}
