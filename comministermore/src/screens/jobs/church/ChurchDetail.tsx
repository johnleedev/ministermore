...
const CHURCH_LOGO_PATH = 'siteimages/churchlogos';
...
export function ChurchDetail({ id, onBack }: Props) {
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<ChurchDetailData | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await axios.post(`${API_BASE}/recruitchurch/getrecruitdatapart`, { id });
        if (!cancelled && res.data?.[0]) {
          const row = res.data[0] as ChurchDetailData;
          setData(row);
    ...
    <RecruitDetailScroll>
      ...
      {(data.address?.trim() || data.location?.trim() || data.locationDetail?.trim()) ? (
        <RecruitDetailMap
          ...
          geocodePath="/recruitchurch/geocode"
