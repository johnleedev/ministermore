...
export function MinisterWrite({ onBack, onSuccess }: Props) {
  ...
  const [partDetailText, setPartDetailText] = useState('');
  ...
  const registerPost = async () => {
    ...
    try {
      ...
      const params = {
        ...
        sort,
        part: JSON.stringify([{ sort: '전임', content: partText }]),
        partDetail: JSON.stringify([{ sort: '전임', content: partDetailText }]),
        recruitNum,
        workday: JSON.stringify([{ sort: '전임', content: '' }]),
        workTimeSunDay: JSON.stringify([{ sort: '전임', startHour: '09', startMinute: '00', endHour: '16', endMinute: '00' }]),
        workTimeWeek: JSON.stringify([{ sort: '전임', startHour: '09', startMinute: '00', endHour: '17', endMinute: '00', day: '평일' }]),
        dawnPray: JSON.stringify([{ sort: '전임', content: '' }]),
        pay: JSON.stringify([{ sort: '전임', paySort: '월', selectCost: '', inputCost: '' }]),
        welfare: JSON.stringify([{ sort: '전임', content: '' }]),
        insurance: JSON.stringify([{ sort: '전임', content: '' }]),
        severance: JSON.stringify([{ sort: '전임', content: '' }]),
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Field label="구분(sort)" value={sort} onChangeText={setSort} />
        <Field label="파트" value={partText} onChangeText={setPartText} />
        <Field label="파트상세" value={partDetailText} onChangeText={setPartDetailText} />
